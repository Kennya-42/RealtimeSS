...
def frames2vid(args):
    ...
    files.sort()
    # print(files[0:10])
    for j in range(len(files)//args.batch_size):
        if j%10 ==0:
            print(j)
        images = get_batch(args.batch_size,files,args,j)
        if images is None:
            break
        with torch.no_grad():
            images = images.cuda()
...
def get_batch(batchsize,files,args,i):
    index = 0
    images = []
    while index < batchsize:
        img = cv2.imread(files[i*args.batch_size+index],1)
        index += 1
